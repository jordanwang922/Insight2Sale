...
- **2026-05-21 成交锦囊**：新增 `/dashboard/deal-kits`，所有角色可见。支持：
  ...
  - **2026-05-21 协议补充**：由于手机端内嵌浏览器下 `Server Action` 仍可能出现 `An unexpected response was received from the server.`，`生成成交话术` 现已改成走普通接口 `POST /api/deal-kits/generate-script`，前端使用 `fetch` 获取 JSON，不再依赖 Server Action 响应协议。
