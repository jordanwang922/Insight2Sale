...
- **2026-05-21 成交锦囊**：新增 `/dashboard/deal-kits`，所有角色可见。支持：
  ...
  - **2026-05-21 修复补充**：顶部说明文案已取消不必要限宽，不会在还有明显空白时提前换行；手工保存增加中文错误兜底；向量入库在方舟失败时自动降级本地哈希，避免保存直接失败；OCR 解析支持 `用 户 画 像 / 用 户 判 断 / 成 交 经 验` 这类带空格标签，并在规则解析不完整时走豆包结构化提取兜底；若三段正文都没识别出，前端直接提示失败，不再假成功。
