...
### 2026-04-12 家长类型 Excel 矩阵列匹配

- 本次目标：图1 类型解读按 Excel **表头列 = 当前养育类型** 只取该列，按行标签逐条展示，禁止 9 型混排。
- 完成内容：新增 `parent-type-matrix.ts`（`xlsx` 读原文件或解析入库 CSV 块）；`interpretation-lookup.ts` 去掉全文截取逻辑；设计文档 §26 更新；`tests/knowledge/parent-type-matrix.test.ts`。
- 验证情况：`npm test` 指定文件、`npm run build` 通过。

### 2026-04-12 工作台类型解读 + 水合修复

- 本次目标：图1 家长类型长文**仅**从知识库标题含「父母养育的9种类型解读」「家长9型解析」的条目中截取**当前类型**段落；修复解读台「复制测评」按钮触发的 **`<p>` 嵌 `<p>`** 水合错误；移除客户页「维度解读」「本客户·口播参考」整块。
- 完成内容：
  - `interpretation-lookup.ts`：`lookupParentTypeSnippet` 限定上述两类标题文档；`extractSingleParentTypeBlock` 按 9 型名称分段；去掉按维度的知识库查询
  - `interpretation-desk-copy-button.tsx`：Toast 容器由 `<p>` 改为 `<div role="status">`
  - `customers/[customerId]/page.tsx`：删除维度解读与口播参考 article；`generateWorkspaceAiOutput` 不再传 `sopParsedSteps`；去掉解读台补充话术表单与 `supplementalBySection` 构建
  - `docs/design/system-design.md`：新增 **§26**；`development-log` 本条
- 验证情况：`npm run build` 通过。

### 2026-04-12 知识库 Excel 上传

- 本次目标：知识库管理支持 **Excel（.xlsx / .xls）** 上传，解析为纯文本后沿用既有切片与向量化；同步设计文档 §25、开发日志与交接日志。
- 完成内容：
  - 依赖 `xlsx`（SheetJS）；`ingestion.ts` 中按工作表 `sheet_to_csv` 合并为 `rawText`（`--- 工作表：名称 ---` 分隔）；`sourceType` 记为 `xlsx`
  - 入库与编辑：`ingest-document.ts`、`server/actions/knowledge.ts` 错误提示与类型分支；`source-type-label.ts` 展示「Excel 文件」
  - 上传页、分类页、条目编辑页、`accept` 增加 `.xlsx`、`.xls`；文案补充 Excel；测评模板创建页同步接受 Excel（复用 `extractKnowledgeText`）
  - `docs/design/system-design.md` 新增 **§25 知识库 Excel 上传与文本化**
- 影响文件：`package.json` / `package-lock.json`、`src/features/knowledge/ingestion.ts`、`ingest-document.ts`、`src/server/actions/knowledge.ts`、`source-type-label.ts`、`src/app/dashboard/knowledge/*`、`assessments/page.tsx`、`docs/design/system-design.md`、`docs/logs/development-log.md`、`docs/logs/handoff-log.md`
- 验证情况：`npm run build` 通过
- 风险 / 注意事项：加密工作簿、空表可能解析失败；大表单次 Server Action 耗时与 PDF 类似，长期可异步化。
...
- 完成内容：
  ...
  - 主管总览页新增“新增销售账号”，默认密码由环境变量配置（后统一为 DEFAULT_NEW_USER_PASSWORD）
